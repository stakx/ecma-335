...
The arguments are passed as though by implicit `starg` (§[III.3.61](#todo-missing-hyperlink)) instructions, see *Implicit argument coercion* §[III.1.6](#todo-missing-hyperlink).
...
In its typical use, `callvirt` is verifiable if:
 ...
 2. the verification type of _obj_ is *verifier-assignable-to* (§[III.1.8.1.2.3](#todo-missing-hyperlink)) with the **this** signature of the method's signature;

 3. the types of the arguments on the stack are *verifier-assignable-to* (§[III.1.8.1.2.3](#todo-missing-hyperlink)) the parameter signatures of the method's signature; and
...
A `callvirt` annotated by `tail.` has additional considerations &ndash; see §[III.1.8](#todo-missing-hyperlink).
