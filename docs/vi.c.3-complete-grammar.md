...
This grammar provides a number of ease-of-use features not provided in the grammar of [Partition II](#todo-missing-hyperlink), as well as supporting some features which are not portable across implementations and hence are not part of this standard. Unlike the grammar of [Partition II](#todo-missing-hyperlink), this one is designed for ease of programming rather than ease of reading; it can be converted directly into a YACC grammar.

#### Lexical tokens

 * _ID_ &ndash; C style alphaNumeric identifier (e.g., `Hello_There2`)
 * _QSTRING_ &ndash; C style quoted string (e.g., `"hi\n"`)
 * _SQSTRING_ &ndash; C style singlely quoted string (e.g., `'hi'`)
