...
This subclause contains text, which is intended for use with the C or C++ preprocessor. By appropriately defining the macros `OPDEF` and `OPALIAS` before including this text, it is possible to use this to produce tables or code for handling CIL instructions. The
`OPDEF` macro is passed 10 arguments, in the following order:

 1. A symbolic name for the opcode, beginning with `CEE_`

 2. A string that constitutes the name of the opcode and corresponds to the names given in [Partition III](#todo-missing-hyperlink).

 3. Data removed from the stack to compute this operations result. The possible values here are the following:

     1. `Pop0` &ndash; no inputs

     2. `Pop1` &ndash; one value type specified by data flow

     3. `Pop1+Pop1` &ndash; two input values, types specified by data flow

     4. `PopI` &ndash; one machine-sized integer

     5. `PopI+Pop1` &ndash; Top of stack is described by data flow, next item is a native pointer

     6. `PopI+PopI` &ndash; Top two items on stack are integers (size can vary by instruction)

     7. `PopI+PopI+PopI` &ndash; Top three items on stack are machine-sized integers

     8. `PopI8+Pop8` &ndash; Top of stack is an 8-byte integer, next is a native pointer

     9. `PopI+PopR4` &ndash; Top of stack is a 4-byte floating point number, next is a native pointer

     10. `PopI+PopR8` &ndash; Top of stack is an 8-byte floating point number, next is a native pointer

     11. `PopRef` &ndash; Top of stack is an object reference

     12. `PopRef+PopI` &ndash; Top of stack is an integer (size can vary by instruction), next is an object reference

     13. `PopRef+PopI+PopI` &ndash; Top of stack has two integers (size can vary by instruction), next is an object reference

     14. `PopRef+PopI+PopI8` &ndash; Top of stack is an 8-byte integer, then a native-sized integer, then an object reference

     15. `PopRef+PopI+PopR4` &ndash; Top of stack is an 4-byte floating point number, then a native-sized integer, then an object reference

     16. `PopRef+PopI+PopR8` &ndash; Top of stack is an 8-byte floating point number, then a native-sized integer, then an object reference

     17. `VarPop` &ndash; variable number of items used, see [Partition III](#todo-missing-hyperlink) for details

 4. Amount and type of data pushed as a result of the instruction. The possible values here are the following:

     1. `Push0` &ndash; no output value

     2. `Push1` &ndash; one output value, type defined by data flow.

     3. `Push1+Push1` &ndash; two output values, type defined by data flow

     4. `PushI` &ndash; push one native integer or pointer

     5. `PushI8` &ndash; push one 8-byte integer

     6. `PushR4` &ndash; push one 4-byte floating point number

     7. `PushR8` &ndash; push one 8-byte floating point number

     8. `PushRef` &ndash; push one object reference

     9. `VarPush` &ndash; variable number of items pushed, see [Partition III](#todo-missing-hyperlink) for details
 ...
 5. Type of in-line argument to instruction. The in-line argument is stored with least significant byte first ("little endian"). The possible values here are the following:
      ...
      1. `InlineBrTarget` &ndash; Branch target, represented as a 4-byte signed integer from the beginning of the instruction following the current instruction.

      2. `InlineField` &ndash; Metadata token (4 bytes) representing a _FieldRef_ (i.e., a _MemberRef_ to a field) or _FieldDef_

      3. `InlineI` &ndash; 4-byte integer

      4. `InlineI8` &ndash; 8-byte integer

      5. `InlineMethod` &ndash; Metadata token (4 bytes) representing a _MethodRef_ (i.e., a _MemberRef_ to a method) or _MethodDef_

      6. `InlineNone` &ndash; No in-line argument

      7. `InlineR` &ndash; 8-byte floating point number

      8. `InlineSig` &ndash; Metadata token (4 bytes) representing a standalone signature

      9. `InlineString` &ndash; Metadata token (4 bytes) representing a UserString

      10. `InlineSwitch` &ndash; Special for the switch instructions, see [Partition III](#todo-missing-hyperlink) for details

      11. `InlineTok` &ndash; Arbitrary metadata token (4 bytes), used for `ldtoken` instruction, see [Partition III](#todo-missing-hyperlink) for details

      12. `InlineType` &ndash; Metadata token (4 bytes) representing a _TypeDef_, _TypeRef_, or _TypeSpec_

      13. `InlineVar` &ndash; 2-byte integer representing an argument or local variable

      14. `ShortInlineBrTarget` &ndash; Short branch target, represented as 1 signed byte from the beginning of the instruction following the current instruction.

      15. `ShortInlineI` &ndash; 1-byte integer, signed or unsigned depending on instruction

      16. `ShortInlineR` &ndash; 4-byte floating point number

      17. `ShortInlineVar` &ndash; 1-byte integer representing an argument or local variable
 ...
 10. Control flow implications of instruction. The possible values here are the following:
     ...
     1. `BRANCH` &ndash; unconditional branch
     2. `CALL` &ndash; method call
     3. `COND_BRANCH` &ndash; conditional branch
     4. `META` &ndash; unused operation or prefix code
     5. `NEXT` &ndash; control flow unaltered ("fall through")
     6. `RETURN` &ndash; return from method
     7. `THROW` &ndash; throw or rethrow an exception
